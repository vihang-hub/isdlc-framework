...
#### Enhancement #2: Scale-Adaptive Workflow Tracks

Workflow tracks that adapt to project complexity, ensuring appropriate rigor without unnecessary overhead.

**Added**
- `isdlc-framework/config/tracks.yaml` - Track definitions and phase mappings
- `docs/SCALE-ADAPTIVE-TRACKS.md` - Documentation for all three tracks
- `.claude/skills/orchestration/assess-complexity.md` (ORCH-009) - Complexity assessment skill
- Track selection during `init-project.sh` (manual or auto-assess)
- `workflow_track` config block in state.json template

**Three Tracks**
- **Quick Flow**: Bug fixes, config changes (Phases 01, 05, 06 only)
- **Standard Flow**: Features, API endpoints (8 phases)
- **Enterprise Flow**: Platforms, compliance systems (all 13 phases)
...
**Features**
- Track-based iteration limits (Quick: 5, Standard: 10, Enterprise: 15)
