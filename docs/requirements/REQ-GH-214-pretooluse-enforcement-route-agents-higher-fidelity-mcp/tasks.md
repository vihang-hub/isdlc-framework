# Task Plan: REQ-GH-214 — PreToolUse Tool Routing
...
- [ ] T0004 Create `src/claude/hooks/config/tool-routing.json` with framework default rules (search-semantic, find-files, file-summary), inference probes, and empty user_overrides | traces: FR-002, AC-002-01
...
- [ ] T0017 Confirm Codex provider is not affected — tool routing is Claude-provider-specific (Codex does not use PreToolUse hooks). Document in architecture-overview.md. | traces: FR-001
...
- [ ] T0018 Copy `tool-router.cjs` to `.claude/hooks/tool-router.cjs` (dogfooding dual-file) | traces: FR-001
  - files: `.claude/hooks/tool-router.cjs` CREATE
  - blocked_by: T0006

- [ ] T0019 Copy `tool-routing.json` to `.claude/hooks/config/tool-routing.json` (dogfooding dual-file) | traces: FR-002
...
- [ ] T0021 Run full test suite (ESM lib + CJS hooks) — ensure no regressions from new hook registration | traces: NFR-001
