...
- [ ] T001 Design test strategy for task-validator, task-reader changes, task-dispatcher extension, and config loading | traces: FR-001, FR-003, FR-004, FR-005
- [ ] T002 Define test cases for traceability enforcement hook and traceability template rendering | traces: FR-006, FR-008

## Phase 06: Implementation -- PENDING

- [ ] T003 Implement task-validator.js with validateTaskCoverage function | traces: FR-001
  files: src/core/tasks/task-validator.js (CREATE)
  blocked_by: []
  blocks: [T008, T010, T012]

- [ ] T004 Remove 3e-plan from isdlc.md Phase-Loop Controller and update BUILD-INIT COPY to require tasks.md | traces: FR-002
  files: src/claude/commands/isdlc.md (MODIFY)
  blocked_by: []
  blocks: [T008]

- [ ] T005 Extend task-reader.js with TNNN regex, parentId derivation, children array, formatTaskContext update, format v3.0 | traces: FR-003
  files: src/core/tasks/task-reader.js (MODIFY)
  blocked_by: []
  blocks: [T006, T007, T008, T014]

- [ ] T006 Implement addSubTask in task-dispatcher.js with parent section lookup and suffix letter assignment | traces: FR-003
  files: src/core/tasks/task-dispatcher.js (MODIFY)
  blocked_by: [T005]
  blocks: [T007, T009, T013]

- [ ] T007 Update markTaskComplete in task-dispatcher.js with sibling check and parent auto-completion | traces: FR-003
  files: src/core/tasks/task-dispatcher.js (MODIFY)
  blocked_by: [T005]
  blocks: [T009]

- [ ] T008 Update Phase-Loop Controller STEP 2 to hydrate Claude TaskCreate from tasks.md via readTaskPlan | traces: FR-004
  files: src/claude/commands/isdlc.md (MODIFY)
  blocked_by: [T003, T004, T005]
  blocks: [T009]

- [ ] T009 Implement show_subtasks_in_ui config reading, conditional TaskCreate for sub-tasks, one-time hint message | traces: FR-004, FR-005
  files: src/claude/commands/isdlc.md (MODIFY), .isdlc/config/config.json (CREATE)
  blocked_by: [T006, T007, T008]
  blocks: []

- [ ] T010 Update roundtable-analyst.md to call validateTaskCoverage before PRESENTING_TASKS and use traceability template | traces: FR-001, FR-008
  files: src/claude/agents/roundtable-analyst.md (MODIFY)
  blocked_by: [T003]
  blocks: [T013]

- [ ] T011 Create traceability.template.json with column definitions and domain scoping rules | traces: FR-008
  files: src/claude/hooks/config/templates/traceability.template.json (CREATE)
  blocked_by: []
  blocks: [T010, T013]

- [ ] T012 Implement traceability-enforcer.cjs hook and register in pre-task-dispatcher chain | traces: FR-006
  files: src/claude/hooks/traceability-enforcer.cjs (CREATE), src/claude/hooks/dispatchers/pre-task-dispatcher.cjs (MODIFY)
  blocked_by: [T003]
  blocks: []

- [ ] T013 Update agent specs 04, 05, 16 with Sub-Task Creation Protocol and add protocol to CLAUDE.md | traces: FR-003, FR-007
...
- [ ] T014 Update tasks.template.json with TNNN ID format and TNNNABC sub-task syntax for format v3.0 | traces: FR-003, FR-008
...
- [ ] T015 Run full test suite, lint, type checks; verify task-reader TNNN format tests; verify task-validator coverage checks | traces: FR-001, FR-003

## Phase 08: Code Review -- PENDING

- [ ] T016 Constitutional review for Article I.5 compliance, dual-file sync, traceability coverage | traces: FR-007
