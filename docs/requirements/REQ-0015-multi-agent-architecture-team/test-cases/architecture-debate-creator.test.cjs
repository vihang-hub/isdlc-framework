/**
 * Tests for Creator Role Awareness in Solution Architect (Module M4)
 * Traces to: FR-004, AC-004-01, AC-004-02, NFR-003
 * Feature: REQ-0015-multi-agent-architecture-team
 * Validation Rules: M4-V01..M4-V06
 *
 * Target file: src/claude/agents/02-solution-architect.md (MODIFIED)
...
const ARCHITECT_PATH = path.resolve(__dirname, '..', '..', '..', '..', 'src', 'claude', 'agents', '02-solution-architect.md');

describe('M4: Creator Role Awareness (02-solution-architect.md)', () => {
    let content;

    function getContent() {
        if (!content) {
            assert.ok(fs.existsSync(ARCHITECT_PATH), 'Solution architect agent file must exist at ' + ARCHITECT_PATH);
            content = fs.readFileSync(ARCHITECT_PATH, 'utf8');
        }
        return content;
    }

    // TC-M4-01: File exists [prerequisite]
    it('TC-M4-01: Solution architect agent file exists', () => {
        assert.ok(fs.existsSync(ARCHITECT_PATH), 'Solution architect agent file must exist');
    });
    ...
    it('TC-M4-08: Backward compatibility -- agent name unchanged', () => {
        const c = getContent();
        assert.ok(
            c.includes('name: solution-architect') || c.includes('solution-architect'),
            'Agent name must remain solution-architect (backward compatibility)'
        );
    });
});
