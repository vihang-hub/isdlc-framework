...
describe('log-skill-usage.js', () => {
    ...
    // =========================================================================
    // BUG-0005: Read-Priority Fix (AC-03c)
    // log-skill-usage must prefer active_workflow.current_phase over top-level
    // =========================================================================

    describe('BUG-0005: active_workflow.current_phase read priority (AC-03c)', () => {

        // TC-03c-01: Prefers active_workflow.current_phase when both set
        it('logs current_phase from active_workflow when both set (divergent)', async () => {
            cleanupTestEnv();
            setupTestEnv({
            ...
            const state = readState();
            const entry = state.skill_usage_log[0];
            assert.ok(entry, 'Should have logged an entry');
            assert.equal(entry.current_phase, '06-implementation',
                'Should use active_workflow.current_phase, not stale top-level');
        });

        // TC-03c-02: Falls back to top-level when active_workflow is null
        it('falls back to top-level current_phase when no active_workflow', async () => {
            ...
            assert.ok(entry, 'Should have logged an entry');
            assert.equal(entry.current_phase, '06-implementation',
                'Should use top-level current_phase when no active_workflow');
        });

        // TC-03c-03: Falls back to default '01-requirements' when both missing
        it('falls back to 01-requirements when both active_workflow and current_phase missing', async () => {
            cleanupTestEnv();
            setupTestEnv();
            ...
            const entry = state.skill_usage_log[0];
            assert.ok(entry, 'Should have logged an entry');
            assert.equal(entry.current_phase, '01-requirements',
                'Should fall back to 01-requirements when both sources missing');
        });

        // TC-03c-04: Prefers active_workflow over extremely stale top-level
        it('prefers active_workflow over extremely stale top-level', async () => {
            cleanupTestEnv();
            setupTestEnv({
                current_phase: '01-requirements',         // EXTREMELY stale
                active_workflow: {
                    type: 'fix',
                    current_phase: '16-quality-loop',     // CORRECT
            ...
            const entry = state.skill_usage_log[0];
            assert.ok(entry, 'Should have logged an entry');
            assert.equal(entry.current_phase, '16-quality-loop',
                'Should use active_workflow.current_phase over extremely stale top-level');
        });
    });
});
