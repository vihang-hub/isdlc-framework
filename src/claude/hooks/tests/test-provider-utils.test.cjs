...
describe('provider-utils.js', () => {
    ...
    // =========================================================================
    // BUG-0005: Read-Priority Fix (AC-03f)
    // selectProvider must prefer active_workflow.current_phase over top-level
    // =========================================================================

    describe('BUG-0005: selectProvider active_workflow.current_phase read priority (AC-03f)', () => {
        ...
        it('uses active_workflow.current_phase for phase routing when both set (divergent)', () => {
            const config = {
                ...pu.getMinimalDefaultConfig(),
                active_mode: 'hybrid',
                phase_routing: {
                    ...
                    '05-test-strategy': {
                        provider: 'anthropic',
                        model: 'sonnet',
                        rationale: 'Cloud for test strategy'
                    }
                }
            };
            const state = {
            ...
            const result = pu.selectProvider(config, state, context);
            // Should route based on 06-implementation (ollama/qwen-coder), not 05-test-strategy
            assert.equal(result.provider, 'ollama',
                'Should use 06-implementation routing from active_workflow, not 05-test-strategy');
            assert.equal(result.model, 'qwen-coder');
            assert.equal(result.source, 'phase_routing');
        });

        // TC-03f-02: selectProvider falls back to top-level
        it('falls back to top-level current_phase when no active_workflow', () => {
            const config = {
                ...pu.getMinimalDefaultConfig(),
                active_mode: 'hybrid',
                phase_routing: {
            ...
            const result = pu.selectProvider(config, state, context);
            assert.equal(result.provider, 'ollama',
                'Should use top-level current_phase for routing when no active_workflow');
            assert.equal(result.model, 'qwen-coder');
        });

        // TC-03f-03: selectProvider falls back to 'unknown' when both missing
        it('falls back to unknown when both active_workflow and current_phase missing', () => {
            const config = {
                ...pu.getMinimalDefaultConfig(),
                active_mode: 'hybrid',
                phase_routing: {
            ...
            const state = {};  // no current_phase, no active_workflow
            const context = { subagent_type: 'software-developer' };

            delete process.env.ISDLC_PROVIDER_OVERRIDE;

            const result = pu.selectProvider(config, state, context);
            // With phase 'unknown', no phase_routing match, should fall to global_default
            assert.ok(result.provider, 'Should still return a provider');
            assert.notEqual(result.source, 'phase_routing',
                'Should NOT match any phase routing with unknown phase');
        });
    });
});
