 ...
 *   update             Update to latest version (global install only)
...
import { checkForUpdates, displayUpdateNotification } from './updater.js';
...
function showHelp() {
  ...
  logger.log('  update             Update to latest version (global install only)');
  ...
  logger.log('  --backup           Create backup before uninstall');
  logger.newline();
  ...
  logger.log('  3. Begin development: /sdlc start');
  logger.newline();
}
...
/**
 * Run update command (for global installs)
 */
async function runUpdate() {
  logger.header('iSDLC Update');

  logger.info('To update iSDLC, run one of the following:');
  logger.newline();
  logger.log('  npm update -g isdlc       # If installed globally');
  logger.log('  npx isdlc@latest init     # Always use latest with npx');
  logger.newline();
