...
// ---------------------------------------------------------------------------
// REQ-0066: Extended ContextNote relationship_hint values (FR-004)
// ---------------------------------------------------------------------------

describe('REQ-0066: Extended ContextNote relationship_hint', () => {
  let tmpDir, projectRoot, userMemoryDir;
  ...
  // MEM-076: accepts builds_on relationship_hint
  it('MEM-076: accepts builds_on relationship_hint', async () => {
    const record = {
      session_id: 'sess_076',
      timestamp: new Date().toISOString(),
      topics: [{ topic_id: 'auth', depth_used: 'standard' }],
      summary: 'Test builds_on hint.',
      context_notes: [{ topic: 'auth', content: 'Builds on past work.', relationship_hint: 'builds_on' }],
    };
    const result = await writeSessionRecord(record, projectRoot, userMemoryDir);
    assert.ok(result.userWritten);
    const sessionsDir = join(userMemoryDir, 'sessions');
    const data = JSON.parse(readFileSync(join(sessionsDir, 'sess_076.json'), 'utf-8'));
    assert.equal(data.context_notes[0].relationship_hint, 'builds_on');
  });

  // MEM-077: accepts contradicts relationship_hint
  it('MEM-077: accepts contradicts relationship_hint', async () => {
    const record = {
      session_id: 'sess_077',
      timestamp: new Date().toISOString(),
      topics: [{ topic_id: 'auth', depth_used: 'standard' }],
      summary: 'Test contradicts hint.',
      context_notes: [{ topic: 'auth', content: 'Contradicts past decision.', relationship_hint: 'contradicts' }],
    };
    const result = await writeSessionRecord(record, projectRoot, userMemoryDir);
    assert.ok(result.userWritten);
    const data = JSON.parse(readFileSync(join(userMemoryDir, 'sessions', 'sess_077.json'), 'utf-8'));
    assert.equal(data.context_notes[0].relationship_hint, 'contradicts');
  });

  // MEM-078: accepts supersedes relationship_hint
  it('MEM-078: accepts supersedes relationship_hint', async () => {
    const record = {
      session_id: 'sess_078',
      timestamp: new Date().toISOString(),
      topics: [{ topic_id: 'auth', depth_used: 'standard' }],
      summary: 'Test supersedes hint.',
      context_notes: [{ topic: 'auth', content: 'Supersedes old approach.', relationship_hint: 'supersedes' }],
    };
    const result = await writeSessionRecord(record, projectRoot, userMemoryDir);
    assert.ok(result.userWritten);
    const data = JSON.parse(readFileSync(join(userMemoryDir, 'sessions', 'sess_078.json'), 'utf-8'));
    assert.equal(data.context_notes[0].relationship_hint, 'supersedes');
  });

  // MEM-079: preserves updates and extends hints (backward compat)
  it('MEM-079: preserves updates and extends hints', async () => {
    const record = {
      session_id: 'sess_079',
      timestamp: new Date().toISOString(),
      topics: [{ topic_id: 'auth', depth_used: 'standard' }],
      ...
      context_notes: [
        { topic: 'auth', content: 'Updates existing info.', relationship_hint: 'updates' },
        { topic: 'arch', content: 'Extends existing info.', relationship_hint: 'extends' },
      ],
    };
    const result = await writeSessionRecord(record, projectRoot, userMemoryDir);
    assert.ok(result.userWritten);
    const data = JSON.parse(readFileSync(join(userMemoryDir, 'sessions', 'sess_079.json'), 'utf-8'));
    assert.equal(data.context_notes[0].relationship_hint, 'updates');
    assert.equal(data.context_notes[1].relationship_hint, 'extends');
  });

  // MEM-080: accepts null relationship_hint
  it('MEM-080: accepts null relationship_hint', async () => {
    const record = {
      session_id: 'sess_080',
      timestamp: new Date().toISOString(),
      topics: [{ topic_id: 'auth', depth_used: 'standard' }],
      summary: 'Test null hint.',
      context_notes: [{ topic: 'auth', content: 'No relationship.', relationship_hint: null }],
    };
    const result = await writeSessionRecord(record, projectRoot, userMemoryDir);
    assert.ok(result.userWritten);
    const data = JSON.parse(readFileSync(join(userMemoryDir, 'sessions', 'sess_080.json'), 'utf-8'));
    assert.equal(data.context_notes[0].relationship_hint, null);
  });

  // MEM-081: invalid values are stored as-is (validation at consumer)
  it('MEM-081: stores invalid hint values (validation at consumer)', async () => {
    const record = {
      session_id: 'sess_081',
      timestamp: new Date().toISOString(),
      topics: [{ topic_id: 'auth', depth_used: 'standard' }],
      summary: 'Test invalid hint.',
      context_notes: [{ topic: 'auth', content: 'Invalid hint.', relationship_hint: 'invalid_type' }],
    };
    // writeSessionRecord accepts it (pass-through). The embedder validates.
    const result = await writeSessionRecord(record, projectRoot, userMemoryDir);
    assert.ok(result.userWritten);
    const data = JSON.parse(readFileSync(join(userMemoryDir, 'sessions', 'sess_081.json'), 'utf-8'));
    assert.equal(data.context_notes[0].relationship_hint, 'invalid_type');
  });
});
