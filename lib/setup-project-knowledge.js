...
/**
 * Install embedding engine dependencies (tree-sitter, onnxruntime-node).
 *
 * @param {string} projectRoot
 * @param {Object} options
 */
async function installEmbeddingDeps(projectRoot, options = {}) {
  ...
  // ONNX runtime — optional, for local CodeBERT inference
  if (!isPackageInstalled('onnxruntime-node')) {
    logger.info('onnxruntime-node not installed — local CodeBERT inference unavailable.');
    logger.info('Cloud embedding providers (Voyage, OpenAI) still work. Install with:');
    logger.info(`  ${buildInstallCommand(pm, 'onnxruntime-node')}`);
  }
}
...
async function generateCodebaseEmbeddings(codePath, options = {}) {
  ...
  try {
    ...
    let embResult;
    try {
      embResult = await embed(texts, { provider: 'codebert' }, {
      ...
      logger.info('Install onnxruntime-node for local embeddings, or configure a cloud provider.');
