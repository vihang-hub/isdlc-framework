...
export async function embed(texts, config, options = {}) {
  if (!texts || !Array.isArray(texts) || texts.length === 0) {
    return {
      vectors: [],
      dimensions: getDimensionsForProvider(config?.provider),
...
function getDimensionsForProvider(provider) {
  switch (provider) {
    case 'codebert': return CODEBERT_DIMENSIONS;
...
export { CODEBERT_DIMENSIONS, VOYAGE_DIMENSIONS, OPENAI_DIMENSIONS };
