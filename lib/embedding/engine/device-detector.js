...
export function resolveConfig(configValues = {}, cliOverrides = {}, env, opts = {}) {
  ...
  const autoParallelism = () => {
    ...
    // FR-003/FR-009/NFR-001: prefer calibrated per-worker cost when present,
    // fall back to hardcoded constants. User max_memory_gb still caps total.
    const perWorker = _resolvePerWorkerMemGB(
      { device, dtype, model: configValues.model },
      projectRoot
    );
    const perWorkerGB = (perWorker && Number.isFinite(perWorker.value) && perWorker.value > 0)
      ? perWorker.value
