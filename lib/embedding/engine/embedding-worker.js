  ...
  port.on('message', async (msg) => {
    ...
    switch (msg.type) {
      case 'batch': {
        ...
        try {
          const forceSequential = opts?.batch_size === 1;
          const vectors = await batchEmbed(extractor, texts, { forceSequential });

          // Convert Float32Array to plain arrays for structured clone transfer
          const plainVectors = vectors.map(v => Array.from(v));

          port.postMessage({
            type: 'result',
            batchIndex,
            vectors: plainVectors,
          });
        } catch (err) {
          port.postMessage({
            type: 'error',
