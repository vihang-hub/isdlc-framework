...
function createTmpProject() {
  ...
  // Use a port guaranteed not in use so serverStatus HTTP probe fails cleanly
  writeFileSync(join(tmp, '.isdlc', 'config.json'), JSON.stringify({
    embeddings: { server: { port: 19999, host: 'localhost' } }
  }));
  return tmp;
}
