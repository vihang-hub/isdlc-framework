...
export async function serverStatus(projectRoot) {
  ...
  // Always check HTTP health — PID file may be stale while server runs under a different PID
  ...
  return {
    running: false,
    pid: pid || undefined,
    reason: !pid ? 'no PID file' : 'PID not alive and server not reachable',
  };
}
