 ...
 * - Jina v2 embedding model (via @huggingface/transformers)
 ...
 * @property {boolean} [skipModel=false] - Skip embedding model setup
...
export async function setupSemanticSearch(projectRoot, options = {}) {
  ...
  const components = {
    ...
    embeddingModel: { installed: false, skipped: false },
  ...
  // 3. Check ONNX runtime — @huggingface/transformers bundles onnxruntime-node transitively.
  // Direct onnxruntime-node is no longer required but we still report if available.
  ...
  if (isPackageAvailable('onnxruntime-node')) {
    ...
    progress(onProgress, 'onnx-runtime', 'skipped', 'onnxruntime-node not directly installed — provided transitively by @huggingface/transformers');
  }

  // 4. Model download — Transformers.js handles caching automatically on first use (Jina v2).
  // No manual download step required. Mark as skipped for component reporting.
  components.embeddingModel.skipped = true;
  progress(onProgress, 'embedding-model', 'skipped', 'Jina v2 model auto-downloads on first use via Transformers.js');
...
export function getSemanticSearchConfig(setupResult) {
  ...
  if (setupResult && setupResult.components) {
    ...
    config.semantic.components = {
      ...
      embeddingModel: c.embeddingModel.installed,
    };

    // Jina v2 via @huggingface/transformers handles local inference.
    // Only fall back to cloud if transformers package is missing entirely.
    if (!c.onnxRuntime.installed && !isPackageAvailable('@huggingface/transformers')) {
      config.semantic.model.provider = 'none';
      config.semantic.model.fallbackNote = 'Install @huggingface/transformers for local inference, or configure a cloud provider';
    }
  }
