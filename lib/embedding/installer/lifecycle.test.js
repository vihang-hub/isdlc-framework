...
describe('FR-004: Installer Embedding Infrastructure', () => {
  ...
  // TC-004-02: Installer source references .isdlc/embeddings/ creation
  it('TC-004-02: installer.js creates .isdlc/embeddings directory', () => {
    const src = readFileSync(join(process.cwd(), 'lib', 'installer.js'), 'utf-8');
    assert.ok(src.includes("'.isdlc', 'embeddings'") || src.includes('.isdlc/embeddings'), 'Installer should reference .isdlc/embeddings directory');
  });
...
describe('FR-005: Uninstaller Embedding Cleanup', () => {
  ...
  // TC-005-02: Uninstaller removes .isdlc/embeddings/ contents
  it('TC-005-02: uninstaller.js cleans .isdlc/embeddings/', () => {
    const src = readFileSync(join(process.cwd(), 'lib', 'uninstaller.js'), 'utf-8');
    assert.ok(src.includes("'.isdlc', 'embeddings'") || src.includes('.isdlc/embeddings'),
      'Uninstaller should reference .isdlc/embeddings directory');
  });
