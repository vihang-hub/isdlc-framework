...
export async function install(projectRoot, options = {}) {
  ...
  if (options.providerMode) {
    ...
    logger.info('Sub-agent routing: quality (default for --force)');
  } else {
    providerMode = await select('Select sub-agent model routing:', [
      { title: 'Quality — Anthropic everywhere (best results, requires API key) (Recommended)', value: 'quality', description: 'Best quality output' },
