...
export async function install(projectRoot, options = {}) {
  ...
  logger.step('1/6', 'Detecting project type...');
  ...
  logger.newline();
  logger.step('2/6', 'Checking for monorepo...');
  ...
  if (options.providerMode) {
    ...
    logger.info(`Provider mode: ${providerMode} (from --provider-mode flag)`);
  } else if (force) {
    providerMode = 'quality';
    logger.info('Provider mode: quality (default for --force)');
  } else {
    providerMode = await select('Select LLM provider mode:', [
      ...
      { title: 'Quality — Anthropic everywhere (best results, requires API key)', value: 'quality', description: 'Best quality output' },
  ...
  logger.newline();
  logger.step('3/6', 'Copying framework files...');
  ...
  if ((await exists(settingsLocalSource)) && !(await exists(settingsLocalTarget))) {
    if (!dryRun) {
  ...
  logger.newline();
  logger.step('4/6', 'Setting up .isdlc folder...');
  ...
  if (await exists(skillsManifestSource)) {
    if (!dryRun) {
      ...
      // Try to convert YAML to JSON (if skills-manifest.json exists in source)
  ...
  // Step 6: Setup docs
  logger.newline();
  logger.step('5/6', 'Setting up docs folder...');
  ...
  logger.newline();
  logger.step('6/6', 'Generating project state...');
  ...
  const modeDescriptions = {
    ...
    quality: 'Anthropic everywhere — best results, requires ANTHROPIC_API_KEY',
  ...
  logger.log(`  Mode: ${providerMode} — ${modeDescriptions[providerMode]}`);
