...
export async function install(projectRoot, options = {}) {
  ...
  // Create or merge .mcp.json (project-local MCP server config)
  const mcpTarget = path.join(projectRoot, '.mcp.json');
  const frameworkMcpServers = {
    'isdlc-embedding': {
      command: 'node',
      args: ['bin/isdlc-embedding-mcp.js'],
    },
  };
  if (!dryRun) {
    try {
      if (await exists(mcpTarget)) {
        const existing = await readJson(mcpTarget).catch(() => ({}));
        if (!existing.mcpServers) existing.mcpServers = {};
        Object.assign(existing.mcpServers, frameworkMcpServers);
        await writeJson(mcpTarget, existing);
        logger.success('Merged .mcp.json (added isdlc-embedding MCP)');
      } else {
        await writeJson(mcpTarget, { mcpServers: frameworkMcpServers });
        logger.success('Created .mcp.json (isdlc-embedding MCP)');
      }
    } catch (err) {
      logger.warning(`Failed to configure .mcp.json: ${err.message}`);
    }
  }
  ...
  // REQ-GH-239 FR-010: gate the `embeddings` block on an install-time opt-in
  // prompt. Default NO. Fail-open on EOF / broken stdin per NFR-006.
  ...
  if (!(await exists(unifiedConfigTarget))) {
    if (!dryRun) {
      ...
      try {
        ...
        } = await import('./install/embeddings-prompt.js');

        // Only fire the interactive prompt on a real TTY — piped installs,
        // CI, and `--force` style flows fall through to the default NO
        // branch (NFR-006, ERR-INSTALL-001).
        if (process.stdin.isTTY && !options?.force) {
          ...
          try {
            enableEmbeddings = await promptEmbeddings(rl);
          } finally {
            try { rl.close(); } catch { /* swallow */ }
          }
        }

        const config = buildInitialConfig({ enableEmbeddings });
        await writeFile(unifiedConfigTarget, JSON.stringify(config, null, 2));
        logger.success(
          enableEmbeddings
            ? 'Created unified config.json (embeddings enabled)'
            : 'Created unified config.json with defaults'
        );
        logger.log(getEmbeddingsPostHint(enableEmbeddings));
      } catch (cfgErr) {
        // Fail-open: write minimal defaults if the helper import fails.
...
`;
  ...
  if (!dryRun) {
    try {
      ...
      const embeddingsDir = path.join(projectRoot, '.isdlc', 'embeddings');
      await ensureDir(embeddingsDir);
      logger.success('Created .isdlc/embeddings/');

      // Model downloads handled automatically by Transformers.js on first use (Jina v2 migration).
      logger.success('Embedding model downloads on first use via Transformers.js');
    } catch (embErr) {
