...
export async function install(projectRoot, options = {}) {
  ...
  for (const scriptName of ['uninstall.sh', 'update.sh']) {
  ...
  logger.section('Agent Model Configuration:');
  logger.log(`  Primary:  Claude Code${claudeCodeFound ? ` (${claudeCodeVersion})` : ''}`);
  const modeDescriptions = {
    'claude-code': 'Claude Code handles all agent work — no extra configuration needed',
    free: 'Free-tier cloud providers (Groq, Together, Google) — requires free API keys',
    budget: 'Ollama locally, free cloud fallback — minimal cost',
    quality: 'Anthropic API everywhere — best results, requires ANTHROPIC_API_KEY',
    local: 'Ollama only — offline/air-gapped, requires GPU with 12GB+ VRAM',
    hybrid: 'Smart per-phase routing — advanced, configure in providers.yaml',
  };
  logger.log(`  Routing:  ${providerMode} — ${modeDescriptions[providerMode]}`);
  logger.log('  Config:   .isdlc/providers.yaml');
  logger.log('  Change:   /provider set <mode>');
  logger.newline();

  // Optional tour
  if (!force && !dryRun) {
    await runTour(select);
  }
  ...
  if (claudeCodeFound) {
    ...
    logger.log('  3. Run `/sdlc start` to begin your workflow');
    ...
    logger.log('     - Research best practices for your stack');
    logger.log('     - Create a tailored constitution interactively');
    logger.log('  4. Run `/sdlc start` to begin your workflow');
...
const tourSections = [
  ...
  },
  {
    ...
    content: [
      '/discover                — Analyze your project or describe a new one',
      '/sdlc feature "desc"    — Develop a feature end-to-end through all phases',
      '/sdlc fix "desc"        — Fix a bug with TDD and tracing agents',
      '/provider               — Configure which LLM models power sub-agents',
    ],
  },
  {
  ...
  },
  {
    ...
    content: [
      'Run /discover to analyze your project',
      'Run /sdlc feature or /sdlc fix to start developing',
      'Run /tour in Claude Code anytime for the full walkthrough',
    ],
  },
  {
 ...
 */
async function runTour(selectFn) {
  logger.header('GET TO KNOW iSDLC');

  const tourChoice = await selectFn('Would you like a quick introduction to the framework?', [
    { title: 'Light intro — 5-minute overview of commands, agents, and workflow', value: 'light' },
    { title: 'Full tour — 15-minute walkthrough of all framework features', value: 'full' },
    { title: 'Skip — Go straight to next steps (you can run /tour later)', value: 'skip' },
  ]);

  if (tourChoice === 'light') {
    for (let i = 0; i < 5; i++) {
      printTourSection(tourSections[i]);
    }
  } else if (tourChoice === 'full') {
    for (let i = 0; i < tourSections.length; i++) {
      printTourSection(tourSections[i]);
      if (i < tourSections.length - 1) {
        const cont = await selectFn('Continue to next topic?', [
          { title: 'Yes — show next section', value: 'yes' },
          { title: 'Skip — jump to next section', value: 'skip' },
          { title: 'Done — end tour', value: 'done' },
        ]);
        if (cont === 'done') {
          logger.newline();
          logger.success('Tour ended. Run /tour in Claude Code anytime to revisit.');
          logger.newline();
          return;
        }
      }
    }
    logger.success('Tour complete! You\'re ready to start.');
    logger.newline();
  } else {
    logger.newline();
    logger.info('Skipped. Run /tour in Claude Code anytime for the introduction.');
    logger.newline();
  }
}
