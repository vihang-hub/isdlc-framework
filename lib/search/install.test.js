...
describe('Search Install', () => {
  ...
  describe('code-index-mcp configuration', () => {
    ...
    it('should configure code-index-mcp MCP server', async () => {
      ...
      assert.ok(configured.includes('code-index'), 'code-index should be configured');
      assert.equal(errors.length, 0);

      const settings = JSON.parse(readFileSync(settingsPath, 'utf-8'));
      assert.ok(settings.mcpServers['code-index']);
      assert.equal(settings.mcpServers['code-index'].command, 'code-index-mcp');
    });

    // TC-002-02: configureMcpServers writes code-index alongside existing backends
    it('should configure code-index alongside existing backends', async () => {
      const settingsPath = join(tmpDir, '.claude', 'settings.json');

      const { configured } = await configureMcpServers(
        [{ id: 'ast-grep' }, { id: 'code-index' }],
        settingsPath,
      );

      assert.ok(configured.includes('ast-grep'));
      assert.ok(configured.includes('code-index'));
