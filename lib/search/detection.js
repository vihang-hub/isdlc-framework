...
const KNOWN_TOOLS = [
  ...
  },
  {
    ...
    versionFlag: '--version',
    installMethods: [
      { method: 'pipx', command: 'pipx install code-index-mcp' },
      { method: 'pip', command: 'pip3 install code-index-mcp' },
    ],
    modality: 'indexed',
    recommendReason: 'Provides sub-second indexed search across the entire codebase using tree-sitter and SQLite FTS5.',
    requiresPython: '3.8',
  },
];

/** Package manager detection */
const PACKAGE_MANAGERS = ['npm', 'cargo', 'brew', 'pipx', 'pip'];

/** Map PM method names to actual binary commands (when they differ) */
const PM_VERSION_COMMANDS = { pip: 'pip3' };
...
export async function detectSearchCapabilities(projectRoot, options = {}) {
  ...
  // Detect Python version (needed for code-index-mcp recommendation)
  const pythonVersion = detectPythonVersion(execFn);
  ...
  const recommendations = generateRecommendations(tools, scaleTier, existingMcpServers, { pythonVersion });
...
function detectPackageManagers(execFn) {
  ...
  for (const pm of PACKAGE_MANAGERS) {
    const cmd = PM_VERSION_COMMANDS[pm] || pm;
    const result = execFn(`${cmd} --version`);
    if (result.success) {
      available.add(pm);
    }
  }
...
/**
 * Detect Python 3 version for tools that require it (e.g., code-index-mcp).
 *
 * @param {Function} execFn - Exec function
 * @returns {string|null} - Semantic version string or null if not available
 */
function detectPythonVersion(execFn) {
  const result = execFn('python3 --version');
  if (result.success) {
    return extractVersion(result.output) || null;
  }
  return null;
}
 ...
 *
 * @param {string} version - e.g., '3.11.4'
 ...
 * @param {string|null} [context.pythonVersion]
 * @returns {ToolRecommendation[]}
 */
function generateRecommendations(tools, scaleTier, existingMcpServers, context = {}) {
  ...
  for (const tool of tools) {
    ...
    // Skip tools requiring Python if Python version is insufficient
    if (def.requiresPython) {
      const { pythonVersion } = context;
      if (!pythonVersion || !meetsMinVersion(pythonVersion, def.requiresPython)) {
        continue;
      }
    }
