...
export function createUserStore(dbPath) {
  ...
  // REQ-0066: Schema migration — add links column if missing (FR-003, AC-003-04)
  try {
    const cols = db.pragma('table_info(memories)');
    const hasLinks = cols.some(c => c.name === 'links');
    if (!hasLinks) {
      db.exec("ALTER TABLE memories ADD COLUMN links TEXT DEFAULT '[]'");
    }
  } catch {
    // MEM-SCHEMA-001: Fall back to store without links column
  }
  ...
  return {
    ...
    /**
     * REQ-0066 FR-006: Batch-fetch chunks by IDs for link traversal.
     * @param {string[]} chunkIds
     * @returns {Promise<object[]>} MemorySearchResult[] in input order, unfound IDs excluded
     */
    async getByIds(chunkIds) {
      if (!chunkIds || chunkIds.length === 0) return [];
      const placeholders = chunkIds.map(() => '?').join(',');
      const rows = db.prepare(`SELECT * FROM memories WHERE chunk_id IN (${placeholders}) AND archived = 0`).all(...chunkIds);
    ...
    /**
     * REQ-0066 FR-003/FR-004/FR-005: Append links to a chunk's links array.
     * @param {string} chunkId
     * @param {object[]} links - MemoryLink[] to append
     * @returns {Promise<void>}
     */
    async updateLinks(chunkId, links) {
      if (!chunkId || !links || links.length === 0) return;
      const VALID_TYPES = new Set(['builds_on', 'contradicts', 'related_to', 'supersedes']);
      const validLinks = links.filter(l => l && VALID_TYPES.has(l.relationType));
      if (validLinks.length === 0) return;
      const row = db.prepare('SELECT links FROM memories WHERE chunk_id = ?').get(chunkId);
      if (!row) return; // No-op for non-existent chunk
      let existing = [];
      try { existing = JSON.parse(row.links || '[]'); } catch { existing = []; }
      existing.push(...validLinks);
      db.prepare('UPDATE memories SET links = ?, updated_at = datetime(\'now\') WHERE chunk_id = ?')
        .run(JSON.stringify(existing), chunkId);
    },
...
export function createProjectStore(embPath) {
  ...
  return {
     ...
     * REQ-0066 FR-006: Batch-fetch chunks by IDs for link traversal.
     * @param {string[]} chunkIds
     * @returns {Promise<object[]>} MemorySearchResult[] in input order
     */
    async getByIds(chunkIds) {
      if (!chunkIds || chunkIds.length === 0) return [];
      const entryMap = new Map(entries.filter(e => !e.archived).map(e => [e.chunkId, e]));
      const results = [];
      for (const id of chunkIds) {
    ...
    async updateLinks(chunkId, links) {
      ...
      if (validLinks.length === 0) return;
      const entry = entries.find(e => e.chunkId === chunkId);
      if (!entry) return; // No-op for non-existent chunk
      if (!entry.links) entry.links = [];
      entry.links.push(...validLinks);
      persistToEmb();
    },
