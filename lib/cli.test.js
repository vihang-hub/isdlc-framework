...
// ───────────────────────────────────────────────
// parseArgs unit tests (REQ-0042 FR-002)
// ───────────────────────────────────────────────
import { parseArgs } from './cli.js';

describe('parseArgs() -- --no-search-setup flag (REQ-0042)', () => {
  // TC-U-017: Recognize --no-search-setup flag
  it('TC-U-017: should recognize --no-search-setup flag', () => {
    const result = parseArgs(['init', '--no-search-setup']);
    assert.equal(result.options.noSearchSetup, true);
    assert.equal(result.command, 'init');
  });

  // TC-U-018: Default noSearchSetup is false
  it('TC-U-018: should default noSearchSetup to false when flag absent', () => {
    const result = parseArgs(['init']);
    assert.equal(result.options.noSearchSetup, false);
  });

  // TC-U-019: Combined with --force
  it('TC-U-019: should work with --force', () => {
    const result = parseArgs(['init', '--force', '--no-search-setup']);
    assert.equal(result.options.force, true);
    assert.equal(result.options.noSearchSetup, true);
  });

  // TC-U-020: Combined with --dry-run
  it('TC-U-020: should work with --dry-run', () => {
    const result = parseArgs(['init', '--dry-run', '--no-search-setup']);
    assert.equal(result.options.dryRun, true);
    assert.equal(result.options.noSearchSetup, true);
  });

  // TC-U-021: Flag order does not matter
  it('TC-U-021: should work regardless of flag order', () => {
    const result = parseArgs(['--no-search-setup', 'init', '--force']);
    assert.equal(result.command, 'init');
    assert.equal(result.options.noSearchSetup, true);
    assert.equal(result.options.force, true);
  });
...
describe('CLI (lib/cli.js) — subprocess tests', () => {
  ...
  describe('help command', () => {
    ...
    it('should show options section in help output', () => {
      ...
      // --provider-mode is disabled (framework is Claude Code-specific)
  ...
  // ───────────────────────────────────────────────
  // --no-search-setup flag (REQ-0042 FR-002)
  // ───────────────────────────────────────────────
  describe('--no-search-setup flag (REQ-0042)', () => {
    // TC-U-022: Help text includes --no-search-setup
    it('TC-U-022: should include --no-search-setup in help output', () => {
      const output = runCLI('help');
      assert.ok(
        output.includes('--no-search-setup'),
        'Help should mention --no-search-setup flag'
      );
    });

    it('TC-U-022: help should describe the flag purpose', () => {
      const output = runCLI('help');
      assert.ok(
        output.includes('search') && output.includes('--no-search-setup'),
        'Help should describe --no-search-setup with search-related description'
      );
    });
  });

  // ───────────────────────────────────────────────
  // init with invalid provider-mode (disabled — framework is Claude Code-specific)
  // ───────────────────────────────────────────────
  // describe('init --provider-mode validation', () => {
  //   ...provider-mode tests removed — feature is disabled...
  // });
});
