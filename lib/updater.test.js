...
// ---------------------------------------------------------------------------
// REQ-0032 FR-006: Updater preserves Issue Tracker Configuration section
// ---------------------------------------------------------------------------

describe('updater: update --force preserves CLAUDE.md (not overwritten)', () => {
  let projectDir;
  let claudeMdBefore;
  let claudeMdAfter;

  before(() => {
    projectDir = setupProjectDir('tracker-preserve');
    runInit(projectDir);
    // Modify CLAUDE.md to simulate user-configured tracker
    const claudePath = join(projectDir, 'CLAUDE.md');
    let content = readFileSync(claudePath, 'utf-8');
    content = content.replace('**Tracker**: manual', '**Tracker**: jira');
    content = content.replace('**Jira Project Key**:', '**Jira Project Key**: MYPROJ');
    writeFileSync(claudePath, content, 'utf-8');
    claudeMdBefore = readFileSync(claudePath, 'utf-8');
    // Run update
    runUpdate(projectDir);
    claudeMdAfter = readFileSync(claudePath, 'utf-8');
  });

  after(() => cleanupTempDir(join(projectDir, '..')));

  // TC-IT-029: Updater preserves Issue Tracker Configuration section
  it('preserves Tracker value after update', () => {
    assert.ok(claudeMdAfter.includes('**Tracker**: jira'), 'Tracker should still be jira after update');
  });

  it('preserves Jira Project Key after update', () => {
    assert.ok(
      claudeMdAfter.includes('**Jira Project Key**: MYPROJ'),
      'Jira Project Key should still be MYPROJ after update'
    );
  });

  it('CLAUDE.md content is unchanged after update', () => {
    assert.equal(claudeMdAfter, claudeMdBefore, 'CLAUDE.md should be identical before and after update');
  });
});

describe('updater: update --force warns when Issue Tracker section is missing', () => {
  let projectDir;
  let output;

  before(() => {
    projectDir = setupProjectDir('tracker-warn');
    runInit(projectDir);
    // Remove the Issue Tracker Configuration section from CLAUDE.md
    const claudePath = join(projectDir, 'CLAUDE.md');
    let content = readFileSync(claudePath, 'utf-8');
    // Remove the section by replacing it with empty
    content = content.replace(/## Issue Tracker Configuration[\s\S]*?(?=## |$)/, '');
    writeFileSync(claudePath, content, 'utf-8');
    output = runUpdate(projectDir);
  });
  ...
  // TC-IT-030: Updater warns when Issue Tracker Configuration section is missing
  it('output warns about missing Issue Tracker section', () => {
    assert.ok(
      output.includes('Issue Tracker Configuration') || output.includes('issue tracker'),
      'output should warn about missing Issue Tracker Configuration section'
    );
  });
});
