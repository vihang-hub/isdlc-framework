...
describe('TC-13: Agent Inventory', () => {
  it('TC-13-01: Exactly 69 agent markdown files exist', () => {
    const files = collectAgentFiles(AGENTS_DIR);
    assert.equal(files.length, 69, `Expected 69 agent files, found ${files.length}: ${files.join(', ')}`);
  });

  it('TC-13-02: Classification counts match expected (1 orch + 3 sub-orch + 15 phase + 28 sub + 22 other = 69)', () => {
    ...
    assert.equal(OTHER_AGENTS.length, 22, 'Other agents');
