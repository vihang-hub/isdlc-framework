...
// ---------------------------------------------------------------------------
// REQ-0014: BACKLOG.md is preserved by uninstaller (TC-16, TC-17, TC-18)
// ---------------------------------------------------------------------------

describe('uninstaller: BACKLOG.md is preserved', () => {
  let projectDir;
  let contentBefore;

  before(() => {
    projectDir = setupProjectDir('backlog-preserve');
    runInit(projectDir);
    contentBefore = readFileSync(join(projectDir, 'BACKLOG.md'), 'utf-8');
  });

  after(() => cleanupTempDir(join(projectDir, '..')));

  // TC-16: BACKLOG.md survives standard uninstall (traces AC-11, FR-04)
  it('BACKLOG.md survives standard uninstall with unchanged content', () => {
    runUninstall(projectDir);
...
describe('uninstaller: BACKLOG.md survives purge-all', () => {
  let projectDir;

  before(() => {
    projectDir = setupProjectDir('backlog-purge');
    runInit(projectDir);
  });

  after(() => cleanupTempDir(join(projectDir, '..')));

  // TC-17: BACKLOG.md survives purge-all uninstall (traces AC-11, FR-04)
  it('BACKLOG.md survives --purge-all uninstall', () => {
    runUninstall(projectDir, '--purge-all');
    assert.ok(existsSync(join(projectDir, 'BACKLOG.md')), 'BACKLOG.md should survive --purge-all');
  });
});

describe('uninstaller: source has zero BACKLOG references', () => {
  // TC-18: Uninstaller source has zero BACKLOG references (traces AC-10, AC-12)
  it('uninstaller.js contains zero references to BACKLOG', () => {
